...
const MAX_RATE_LIMIT_ATTEMPTS = 3; // Số lần thử tối đa

function checkRateLimit(proxyHost) {
    const now = Date.now();
    const limits = rateLimitMap.filter(limit => 
        limit.proxy === proxyHost && (now - limit.timestamp) < RATE_LIMIT_WINDOW
    );
    return limits.length >= MAX_RATE_LIMIT_ATTEMPTS;
}

function markRateLimited(proxyHost) {
    rateLimitMap.push({ proxy: proxyHost, timestamp: Date.now() });
    // Giới hạn kích thước map
    if (rateLimitMap.length > 1000) {
        rateLimitMap.splice(0, 100);
    }
}

function cleanupRateLimitMap() {
    const now = Date.now();
    for (let i = rateLimitMap.length - 1; i >= 0; i--) {
        if (now - rateLimitMap[i].timestamp > RATE_LIMIT_WINDOW) {
            rateLimitMap.splice(i, 1);
        }
    }
}

// Cleanup mỗi 30 giây
setInterval(cleanupRateLimitMap, 30000);
...
function updateProxyHealth(proxy, success, responseTime = 0) {
  if (!proxyHealth[proxy]) {
    proxyHealth[proxy] = { success: 0, fail: 0, avgTime: 0, score: 1.0, rateLimitCount: 0 };
  }
...
const options = [
  ...
  { flag: '--no-rate-limit', value: get_option('--no-rate-limit'), default: false },
];
...
var noRateLimit = enabled('no-rate-limit');
...
async function main(reserve) {
  return new Promise(async (resolve) => {
    ...
    // ===== RATE LIMIT CHECK =====
    while (!noRateLimit && checkRateLimit(proxy)) {
        attempts++;
        if (attempts >= maxAttempts) {
            if (debug) log(1, `All proxies rate limited, waiting 5s...`);
            await sleep(5000);
            cleanupRateLimitMap();
            attempts = 0;
            continue;
    ...
    try {
      ...
      try {
        ...
        // Nếu lỗi là do rate limit, đánh dấu proxy
        if (err.message && err.message.includes('429')) {
            markRateLimited(proxy);
        }
      ...
      // Đánh dấu rate limit nếu lỗi liên quan
      if (err.message && (err.message.includes('429') || err.message.includes('Too Many Requests'))) {
          markRateLimited(proxy);
          log(1, `(${colors.magenta(proxy)}) ${colors.bold('Rate Limited')}: Marked for 60s`);
      }
